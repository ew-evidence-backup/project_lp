...
class lazy_mofo {
    ...
    public $grid_sql = ''; // optional, render grid with desired sql. defaults to 'select *, identity_name from table'. *important* to display [edit] and [delete] links, the identity must be the last column in the sql statement. this arangement allows grid to display data without showing the identity.
    ...
    public $thumb_path = ''; // optional, leave blank if you don't need thumbnails
    ...
    public $date_out_mysql = '%m/%d/%Y'; // used for search, output date in mysql format, be sure to match date_out
    public $datetime_out_mysql = '%m/%d/%Y %h:%i %p'; // used for search, output datetime in mysql format, be sure to match datetime_out
    ...
    public $grid_limit = 25; // pagination limit number of records per page
    ...
    public $form_back_button = "<input type='hidden' value='&lt Back/Cancel' class='lm_button' onclick='_back();'; />"; // use type=button for delete and cancel so form presses the right button with enter key
    public $form_delete_button = "<input type='button' value='Delete' class='lm_button' onclick='_delete();' />";

    public $form_text_record_saved = 'Saved'; // customize success messages
    public $form_text_record_added = 'Added';
    ...
    public $child_grid_add_link = "<a href='#' onclick='return _child_add()' class='lm_child_grid_add_link'>Add a Record</a>";

    public $grid_text_record_added = "Added";
    public $grid_text_changes_saved = "Saved";
    public $grid_text_record_deleted = "Deleted";
    public $grid_text_save_changes = "Save Changes";
    public $grid_text_search = "Search";
    public $grid_text_delete = "Delete";
    public $grid_text_no_records_found = "Nothing Found";
    ...
    */
    ...
    function form($error = '') {
        ...
        $html .= "<table cellpadding='5' cellspacing='5' border='0' width='100%' class='form-group'>\n";
        ...
        foreach ($columns as $column_name) {
            ...
            $html .= "<tr>\n";
            $html .= "    <td class='lm_label'>$title:</td>\n";
            $html .= "    <td>$control</td>\n";
            $html .= "</tr>\n";
        }
        ...
        // default action in case browser doesn't pickup action from button
        if ($action == 'edit')
    ...
    function child_grid($identity_id, $url, $error = '') {
        ...
        foreach ($result as $row) {
            ...
            $table_body .= "<tr class='lm_shaded'><td align='center'><input type='checkbox' name='_delete[]' value='{$row[$this->child_identity_name]}' /></td>$cells</tr>\n";

        }

        // add a record - blank entry code
        $mutli_upload_added = false;
        $table_body_add = "<div><a href='#' onclick='return _child_remove(this.parentNode.parentNode)' title='Cancel' class='lm_child_delete'>[x]</a></div>";
        foreach ($columns as $column_name) {
        ...
        <table cellpadding='2' cellspacing='1' border='0' width='100%' class='lm_child_grid' id='lm_child_grid'>
        $table_header
        $table_body
        </table>
    ...
    function grid($error = '') {
        ...
        if ($result === false) {
            if ($_REQUEST['_search'] != '')
                echo "<br><br>Sorry, unable to create a search query. If this error only occurs when searching please read \"Customizing the Search Query\" in the documention for a work-around on this issue.";
            return;
        }
        ...
        if ($_pagination_off == 1) {
            ...
            if ($result_count === false && mb_strlen($this->grid_sql_count) == 0) {
                echo "<div class='lm_error'><b>Sorry, unable to parse grid_sql for counting the results. Try adding a grid_sql_count statement.<br />Example: \$lm-&gt;grid_sql_count = 'select count(1) as count from table';</div>";
                return;
            }
        ...
        if (count($this->grid_input_control) > 0 || $this->grid_multi_delete == true)
            $button = "<input type='submit' name='__update_grid' value='$this->grid_text_save_changes' class='lm_save_changes_button' />";
        ...
        if ($this->grid_show_search_box)
            $search_box = "<form action='$uri_path' method='post' class='lm_search_box'><input type='text' name='_search' value='" . $this->clean_out($_search) . "' size='20' class='lm_search_input' /> <input type='submit' value='$this->grid_text_search' class='lm_search_button' /><input type='hidden' name='action' value='search'  /><input type='hidden' name='_csrf' value='$_SESSION[_csrf]' /></form>";
    ...
    function get_columns($context = '') {
        ...
        if ($context == 'grid') {
            ...
            $table = $this->child_table;
            if ($sql == '')
                $sql = "select * from `$this->child_table`";
        } else {
            $sql = $this->form_sql;
            $sql_param = $this->form_sql_param;
            $table = $this->table;
